...
<body>
  ...
  if (!$result2) {
    echo "Ocurrió un error con query (Archivo: crear_reporteB.php, nombreg).\n";
    exit;
  }
  ...
  if (!$result3) {
    echo "Ocurrió un error con query (Archivo: crear_reporteB.php, info alumno).\n";
    exit;
  }

  /***************************** INFO CURSO *********************************/
  $sql_curso = "SELECT nombrec FROM curso WHERE nombregrupo = '$nombreg'";
  $result4 = pg_query($dbconn, $sql_curso);
  if (!$result4) {
    echo "Ocurrió un error con query (Archivo: crear_reporteB.php, info curso).\n";
    exit;
  }
  ?>
  <center>
    <h2 class = "bienvenido__tutor">Crear reporte Bimestral</h2>
    Para el campo "fecha" ingrese una fecha con el mes del bimestre que desea realizar el reporte.
  </center>
  <br>
  <br>
  <form action= "creando_reporteB.php" method="post">
    <center>
      FECHA: <input type="date" class="login-input" name="fecha" autofocus required="">
      <br>
      CURSO:
      <select name ="actividad" class="login-input">
