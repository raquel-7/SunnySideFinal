...
if (!$dbconn) {
  echo "Ocurrió un error con la conexion. (Archivo: tareas.php)\n";
  exit;
}
...
<script src="../js/menu.js"></script>
  ...
  if (!$result0) {
    echo "Ocurrió un error con query (Archivo: tareas.php, nombreg).\n";
    exit;
  }
  ...
  /****************************** TAREAS *****************************************/
  $sql_tareas = "SELECT * FROM tareas WHERE nombreg = '$nombre_grupo'";
  $result = pg_query($dbconn, $sql_tareas);
  if (!$result) {
    echo "Ocurrió un error con query (Archivo: tareas.php, tareas).\n";
    exit;
  }
